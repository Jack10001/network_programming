...
int main(int argc, char* argv[])
{
    ...
    sigaction(SIGCHLD, &act, 0);//信号注册函数，一旦子进程执行完毕触发信号，操作系统就会调用sigaction函数

    pid_t pid = fork();//父进程负责accept客户端的连接请求,子进程负责将客户端的信息保存到文件中，进程间的通信通过管道来实现
    if(pid == 0){
        char buf[32];
   ...
   struct sockaddr_in clientAddr;
    ...
    while(1){
        if((sockClient = accept(sockServ, (struct sockaddr*)&clientAddr, &clientAddrLen)) == -1){
            continue;
